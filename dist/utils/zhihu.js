...
exports.getFinalHTML = getFinalHTML;
...
async function getQuestionDetail(questionId) {
    try {
        const res = await (0, axios_1.default)(`https://www.zhihu.com/api/v4/questions/${questionId}?include=data%5B*%5D.excerpt,content,detail`);
        return res.data;
    }
    catch (err) {
        return {};
...
function getFinalHTML({ title, html, gutter }) {
    return `<!DOCTYPE html>
  <html lang="en">
    <head>
      <meta charset="UTF-8" />
...
async function getResult({ $, contentNode, query, wrapHTML, title, detail }) {
    console.log("query", query);
    const { url, markdown, imagePrefix: isImagePrefix, markdownArray, markdownHTML, latexWrap = "$$" } = query;
                ...
                if (el.type === "text") {
                    ...
                    $(el)
                        ...
                        if (a.attribs.class === "video-box") {
                            const url = decodeURIComponent(a.attribs.href).match(/https?\:\/\/link\.zhihu\.com\/\?target=(.*)/)?.[1];
                            url &&
                                $(a).replaceWith(`[${$(a).find(".title")?.text().replace(/\[|\]/g, "") || " "}](${url})`);
                        }
                        else {
                            // 外链
                    ...
                    if (el.name === "a") {
                        if (el.attribs.class === "video-box") {
                            return `[${$(el).find(".title")?.text().replace(/\[|\]/g, "") || " "}](${el.attribs.href})`;
                        }
